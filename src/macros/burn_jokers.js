...
if(errMsg == "") {
    if(!redJoker) {
        ...
        if(!game.decks.deckCheck(redJoker.id,mDeck.deckID) )     ui.notifications.notify("Red Joker not found in macro deck."); 
    }
    ...
    if(!blackJoker) {
        ...
        if(!game.decks.deckCheck(blackJoker.id,mDeck.deckID) )     ui.notifications.notify("Black Joker not found in macro deck."); 
    }
    discardJokers();
