...
export class cardHotbar extends Hotbar {
  ...
  _getcardMacrosByPage(page) {
    ...
    for (let [i, m] of macros.entries()) {
      ...
      let sw = m.macro
        ? m.macro.getFlag("world", "scaledWidth") || defaultWidth
        : defaultWidth;
      m.scaledWidth = sw ? sw : defaultWidth;
      this.totScaledWidth += m.scaledWidth;
      let defaultSide = "front";
      let curSide = m.macro
        ? m.macro.getFlag("world", "sideUp") || defaultSide
        : defaultSide;
      m.sideUp = curSide ? curSide : defaultSide;
      let defaultMark = 0;
      let curMark = m.macro
        ? m.macro.getFlag("world", "marked") || defaultMark
        : defaultMark;
      ...
      if (m.cssClass == "inactive" && lastCard == false) {
        if (macros[i - 1] && macros[i - 1].macro) {
          macros[i - 1].cssClass == "active marked"
            ? (macros[i - 1].cssClass = "last marked")
      ...
      if (lastCard == false && i == 53 && m.macro) {
        m.cssClass == "active marked"
          ? (m.cssClass = "last marked")
          : (m.cssClass = "last");
        lastCard = true;
      }
    }
  ...
  async assigncardHotbarMacro(macro, slot, { fromSlot = null } = {}) {
    ...
    slot = slot
      ? parseInt(slot)
      : Array.fromRange(54).find((i) => !(i in ui.cardHotbar));
        ...
        callback: (li) => {
          if (game.user.isGM) {
            const journal = game.journal.get(
              game.macros.get(li.data("macro-id")).getFlag("world", "cardID")
            );
        ...
        callback: async (li) => {
          ...
          try {
            ...
            if (!game.user.isGM) {
              ...
              game.decks
                .getByCard(macro.data.flags.world.cardID)
                .discardCard(macro.data.flags.world.cardID);
            }
  ...
  activateListeners(html) {
    ...
    html
      .find("#chbTakeFromPlayer")
      .click(this.populator.takeFromPlayer.bind(this));
    html.find("#chbInteractDecks").click((ev) => {
      new DeckForm().render(true);
    });
  ...
  async _onDrop(event) {
    ...
    if (data.type == "JournalEntry") {
      ...
      if (je) {
        ...
        await this.assigncardHotbarJE(je, cardSlot, {
          fromSlot: data.cardSlot,
        });
      }
      return;
    }
    ...
    if (macro) {
      console.debug(
        "Card Hotbar | macro provided:",
        macro,
        "fromSlot",
        data.cardSlot
      );
      console.debug(
        "Card Hotbar | monkey hotpatch?",
        game.user.assignHotbarMacro === this.assigncardHotbarMacro
      );
      ...
      console.debug(
        `Card Hotbar | From ${
          data.cardSlot
        } (${ui.cardHotbar.populator.chbGetMacro(
          data.cardSlot
        )}) to ${cardSlot} (${ui.cardHotbar.populator.chbGetMacro(cardSlot)})`
      );
      let dstMacro = game.macros.get(
        ui.cardHotbar.populator.chbGetMacro(cardSlot)
      );
  ...
  _onHoverMacro(event) {
    ...
    if (event.type === "mouseenter") {
      ...
      if (hasAction) {
        ...
        let curDeck =
          game.folders.get(game.user.getFlag("world", "sdf-deck-cur")).name ||
          "None";
        tooltip.classList.add("tooltip");
