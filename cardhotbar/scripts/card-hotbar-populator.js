...
export class cardHotbarPopulator {
  ...
  async addToPlayerHand(cards) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (game.user.getFlag("cardsupport", "chbDrawFaceUpHand") !== undefined) {
        defaultSideUp =
          game.user.getFlag("cardsupport", "chbDrawFaceUpHand") == true
            ? "front"
            : "back";
      } else {
        defaultSideUp =
          game.settings.get("cardsupport", "chbDrawFaceUpHand") == true
            ? "front"
            : "back";
      }
      ...
      if (maxSlot >= i + firstEmpty) {
        ...
        macro = await Macro.create({
          ...
          img:
            sideUp == "front"
              ? journal.data.img
              : journal.getFlag("world", "cardBack"),
        });
        console.debug("Prepping new macro to add...");
        tempCardMacros[firstEmpty + i] = macro.id;
      } else {
        ui.notifications.error(
          "Not enough space in hand, at least 1 card not added."
        );
        ui.cardHotbar.render();
        return -1;
      }
  ...
  async flipCard(slot) {
    ...
    if (mm.data.img == frontImg) {
      ...
      ui.notifications.error(
        "What you doing m8? Stop breaking Spaceman's code that Norc stole..."
      );
      return sideUp;
    }
  ...
  compact() {
    let filled = duplicate(
      this.macroMap.filter(function (card) {
        return card != null;
      })
    );
    filled.unshift(null);
  ...
  async _updateFlags() {
    ...
    const result = await game.user.setFlag(
      "cardsupport",
      "chbMacroMap",
      this.macroMap
    );
